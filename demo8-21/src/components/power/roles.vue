			...
			<el-table :data="rolelist" border stripe>
				...
				<el-table-column type="expand">
					<template slot-scope="scope">
						<el-row :class="['bdbottom',il===0 ? 'bdtop':'bdbottom','verticall']" v-for="(item,il) in scope.row.children" :key="item.id">
							<!-- 渲染一级权限 -->
							<el-col :span="5">
				...
				<el-table-column label="操作" width="290px">
					<template slot-scope='scope'>
						...
						<el-button type="warning" icon="el-icon-setting" size="mini" @click='showSetRigthDialog(scope.row.id)'>分配权限</el-button>
					</template>
				</el-table-column>
			</el-table>
		</el-card>
		...
		methods: {
			...
			addDialogVisibleok(){
				this.$refs.addrolesFormRef.validate(async value =>{
					...
					const {data:res} = await this.$http.post('roles',{roleName:this.addroleAnList.addroleName,roleDesc:this.addroleAnList.addroleDesc})
			...
			async amendRole(id) {
				...
				if(res.meta.status!=200){ return this.$message.error('获取编辑信息失败')}
			...
			rolesDialogVisibleOk(){
				this.$refs.rolesFormRef.validate(async value=>{
					if(!value){return this.$message.error('角色编辑失败！')}
					const {data:res} = await this.$http.put('roles/'+this.roleAnList.roleId,{
			...
			async deleteRole(id){
				...
				if(deleteRl !== 'confirm'){return this.$message.info('你取消了此操作')}
			...
			async removeRoles(roles,rightId){
				const reomvRl= await this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
				          confirmButtonText: '确定',
				          cancelButtonText: '取消',
				          type: 'warning'
				        }).catch(err=>err)
						//判断返回的是值
						if(reomvRl !== 'confirm'){return this.$message.info('你取消了删除！')}
			...
			//控制分配权限按钮的显示与隐藏
			async showSetRigthDialog(id){
				//获取所有权限数据
				const {data:res} = await this.$http.get('rights/tree')
				if(res.meta.status != 200){return this.$message.error('分配权限获取失败')}
